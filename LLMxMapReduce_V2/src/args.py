...
def parse_args():
    ...
    parser.add_argument("--max_results", type=int, default=30, help="Maximum number of results to return from arXiv API per query")
    ...
    parser.add_argument("--parallel_num", type=int, default=4, help="Number of data to process concurrently in pipeline") # 原默认值是1
    
    parser.add_argument("--digest_group_mode", type=str, choices=["random", "llm"], default="llm", help="Group paper to digest mode")
    parser.add_argument("--skeleton_group_size", type=int, default=1, help="Number of digest to generate a skeleton")   # 原默认值是3
    
    parser.add_argument("--block_count", type=int, default=2, help="Number of max iteration blocks")    # 原默认值是0
    parser.add_argument("--output_each_block", type=bool, default=False, help="Whether to output each block")
    
    parser.add_argument("--conv_layer", type=int, default=3, help="Number of convolution layer, only mcts need.")  # 原默认值是6
    ...
    parser.add_argument("--conv_result_num", type=int, default=5, help="Number of suggestion to generate in each convolution layer")    # 原默认值是10
    parser.add_argument("--top_k", type=int, default=6, help="Number of top k suggestion that will be kept in mcts")
    
    parser.add_argument("--self_refine_count", type=int, default=1, help="Number of self-refine cycle")  # 原默认值是3
